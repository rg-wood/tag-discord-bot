...
module.exports = new Clapp.Command({
  name: "remove",
  desc: "remove a tag from user nick",
  fn: (argv, context) =>
    new Promise((resolve, _) => {
      ...
      resolve(
        `Removing tag "${argv.args.tag}" from user ${member.displayName}'s nick.`
      );

      const nick = new TaggedNick(member.displayName).removeTag(argv.args.tag);
